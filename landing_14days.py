...
def p_14days(lang):
    ...
    return {
        'ko': dict(
            ...
            desc='테스터 12명이 14일을 채워야 출시할 수 있습니다. 구글이 세는 것, 테스터가 실제로 앱을 여는 날수, 며칠째에 사람이 빠지는지, 14일 뒤 신청 요령까지. 끝난 테스트 48건 실측.',
            h1='비공개 테스트 14일, 실제로는 이렇게 흘러갑니다',
            sub='끝난 테스트 48건을 날짜별로 기록한 결과',
            body=f'''
<p>구글 플레이 콘솔은 새 개발자 계정에 <b>테스터 12명, 14일</b>을 요구합니다.
조건은 한 줄인데, 그 14일 동안 무엇을 해야 하고 무엇이 세어지는지는 어디에도
없습니다. 개발자끼리 테스트를 교환하는 보드에서 끝난 테스트 48건의 매일 기록을
모았습니다.</p>
...
<h2>테스터는 14일 중 8.8일만 엽니다</h2>
<p>48건 평균입니다. 14일을 전부 연 테스터는 48명 중 5명뿐이었습니다.
...
아무것도 보고할 필요가 없습니다.</p>''',
            ...
            faq=[
                ...
                ('테스터가 앱을 안 열면 심사에 떨어지나요?', '아닙니다. 구글은 옵트인 기간을 봅니다. 실측 48건에서 평균 8.8일만 열었고 통과했습니다. 다만 설문에서 테스터 참여도를 물으니 아예 안 여는 건 좋지 않습니다.'),
        ...
        'en': dict(
            ...
            desc='12 testers must stay opted in for 14 days before you can publish. What Google counts, how many days testers really open the app, when people drop out, and how to apply on day 14. From 48 finished tests.',
            h1='The 14 days of closed testing, as they really happen',
            sub='Daily records from 48 finished tests',
            body=f'''
...
daily record of 48 finished runs.</p>
...
<h2>Testers open the app on 8.8 of 14 days</h2>
<p>That is the average across 48 runs. Five of 48 testers opened it every
...
what on the board; the tester reports nothing.</p>''',
            ...
            faq=[
                ...
                ("Do I fail review if testers don't open the app?", 'No. Google counts opt-in time. In 48 measured runs testers averaged 8.8 open days and passed. The production form does ask about engagement, so zero opens is still a bad look.'),
...
テスターは何も報告しません。</p>''',
        ...
        'zh-Hans': dict(
            ...
            desc='12 名测试者必须保持加入状态 14 天才能发布。Google 到底在数什么、测试者真正打开应用的天数、人会在第几天流失、第 14 天怎么申请。来自 48 次已完成的测试。',
...
测试者什么都不用汇报。</p>''',
            ...
            faq=[
                ...
                ('测试者不打开应用会审核不过吗？', '不会。Google 看的是加入时长。实测 48 次平均只打开 8.8 天，都通过了。不过申请表会问参与度，完全不打开也不好看。'),
        ...
        'zh-Hant': dict(
            ...
            desc='12 名測試者必須保持加入狀態 14 天才能發布。Google 到底在數什麼、測試者真正打開應用程式的天數、人會在第幾天流失、第 14 天怎麼申請。來自 48 次已完成的測試。',
...
測試者什麼都不用回報。</p>''',
            ...
            faq=[
                ...
                ('測試者不打開應用程式會審核不過嗎？', '不會。Google 看的是加入時長。實測 48 次平均只打開 8.8 天，都通過了。不過申請表會問參與度，完全不打開也不好看。'),
